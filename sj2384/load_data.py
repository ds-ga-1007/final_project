...
def safely_input():
    ...
    """
    print ("Welcome! \n")
    path = "LoanStats3a.csv"
    while 1:
        try:
            raw_data = get_input(path)
            break
        except IOError:
            path = input( "Can not locate the dataset file, type in a valid path or type 'quit' to exit \n")
        except Wrong_dataset_exception:
            path = input( "Load a wrong dataset, please type in a valid path or type 'quit' to exit \n")
        if path == 'quit':
            sys.exit()
    return raw_data
...
def get_input(path):
    ...
    feats = ['id', 'member_id', 'loan_amnt', 'funded_amnt', 'funded_amnt_inv',
